...
(func_example 5)
```
В данном примере ```func_example``` является именем функции, ```x``` - аргументом, имеющим тип float. Условие ```if-else``` представляет собой тело функции. Возвращаемое значение имеет тип float. 
